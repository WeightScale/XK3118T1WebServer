...
public:
	...
	Task(unsigned long _interval = 0, bool single = false);
	//Task(void (*callback)(void) = NULL, unsigned long _interval = 0);
	Task(void(*callback)(void) = NULL, unsigned long _interval = 0, bool single = false);
	virtual ~Task() {};
	...
	//void kill(bool kill) {_kill = kill;};
